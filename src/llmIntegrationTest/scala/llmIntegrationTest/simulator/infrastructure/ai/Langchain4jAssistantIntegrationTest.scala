...
class Langchain4jAssistantIntegrationTest
    extends AnyFunSuite
    with Matchers
    with MockFactory
    with BeforeAndAfterEach:

  val circleEntity: Entity = Entity
    .circle("e1", Vector2D(20, 20), 50)
    .value
    .withWeight(Some(70))
    .value
    .withSpeed(Some(Vector2D(20, 35)))

  val rectangleEntity: Entity = Entity
    .rectangle("e2", Vector2D(4, 3), 11, 9)
    .value
    .withWeight(Some(20))
    .value
  ...
  private def getAssistant: Langchain4jAssistant =
    Langchain4jAgentFactory
      .buildOllama(ollamaConfig)(using
        world,
        gameEngineRuntime
      )
      .assistant
  ...
  test("Can create a circular entity using the assistant when game engine is in pause"):
    val userMessage =
      "Create a circle entity with id e1 with center in 20,20 and a radius of 50, his weight is 70, and a speed of 20,35"
    val assistant = getAssistant
  ...
  test("Assistant can talk only about The application tools and geometry, not on any other things"):
    ...
    val judgeCriteria =
      "Assistant should refuse to answer user question cause it can talk about his allowed topics"
    result judgedBy judgeAssistant shouldSatisfy judgeCriteria
    result should notExecuteTools
  ...
  test("Assistant should response with same languages as user question"):
    ...
    val judgeCriteria =
      "Assistant should response in same language as user question, Italian in this case"
    result judgedBy judgeAssistant shouldSatisfy judgeCriteria
    result should notExecuteTools
