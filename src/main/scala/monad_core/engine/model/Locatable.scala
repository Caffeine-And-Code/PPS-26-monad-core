...
object Locatable:
  def circle[A](id: String, position: Vector2D, radius: Double, rotation: Double = 0)(build: (LocatableId, Vector2D, Shape2D, Double) => A): Either[EngineError, A] =
  ...
  def rectangle[A](id: String, position: Vector2D, height: Double, length: Double, rotation: Double = 0)(build: (LocatableId, Vector2D, Shape2D, Double) => A): Either[EngineError, A] =
    Shape2D.rectangle(height, length).flatMap(rectangle => createGeneric(id, position, rectangle, rotation)(build))
