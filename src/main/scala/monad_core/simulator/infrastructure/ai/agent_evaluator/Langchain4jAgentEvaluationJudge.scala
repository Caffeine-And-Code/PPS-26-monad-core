...
import monad_core.simulator.infrastructure.ai.Langchain4jToolResponse.{renderEntity, renderSurface, renderTeam}
...
object Langchain4jAgentEvaluationJudge:
  ...
  def build(chatModel: ChatModel): Langchain4jAgentEvaluationJudge =
    val assistant = AiServices.builder(classOf[Langchain4jAgentEvaluationJudgeAssistant])
      .chatModel(chatModel)
      .build()
