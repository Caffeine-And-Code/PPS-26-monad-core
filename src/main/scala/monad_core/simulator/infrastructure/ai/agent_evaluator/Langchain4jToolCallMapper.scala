...
case class Langchain4jToolCallMapper():
  ...
  def from(request: ToolExecutionRequest): Either[BaseError, ToolCall] =
    request.name() match
      ...
      case "createCircleEntity" => withArguments(request)(arguments => createCircleEntity(arguments))
      case "createRectangleEntity" => withArguments(request)(arguments => createRectangleEntity(arguments))
      case "updateCircleEntity" => withArguments(request)(arguments => updateCircleEntity(arguments))
      case "updateRectangleEntity" => withArguments(request)(arguments => updateRectangleEntity(arguments))
      ...
      case "createCircleSurface" => withArguments(request)(arguments => createCircleSurface(arguments))
      case "createRectangleSurface" => withArguments(request)(arguments => createRectangleSurface(arguments))
      case "updateCircleSurface" => withArguments(request)(arguments => updateCircleSurface(arguments))
      case "updateRectangleSurface" => withArguments(request)(arguments => updateRectangleSurface(arguments))
  ...
  private def withArguments(
    ...
    parseArguments(request).flatMap(mapper).left.map:
  ...
  private def parseArguments(request: ToolExecutionRequest): Either[BaseError, Arguments] =
    Try {
      ...
      ).asScala.toMap.asInstanceOf[Arguments]
  ...
  private def optionalString(arguments: Arguments, name: String): Either[BaseError, Option[String]] =
    arguments.get(name) match
  ...
  private def optionalDouble(arguments: Arguments, name: String): Either[BaseError, Option[Double]] =
    arguments.get(name) match
      case None | Some(null) => Right(None)
