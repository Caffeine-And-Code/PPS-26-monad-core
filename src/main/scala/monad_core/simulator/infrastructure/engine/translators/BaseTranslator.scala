...
import monad_core.engine.model.{
  Entity as EngineEntity,
  Shape2D as EngineShape,
  Surface as EngineSurface,
  Team as EngineTeam
}
...
object BaseTranslator:
  ...
  def determineShape(shape2D: EngineShape): MonadCoreShape =
    shape2D match
      ...
      case EngineShape.Rectangle(height, length) =>
        SimulationRectangle(width = length, height = height)
