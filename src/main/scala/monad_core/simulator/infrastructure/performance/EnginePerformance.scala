...
import monad_core.simulator.application.performance.PerformanceRunner
...
object EnginePerformance:
   ...
   * @see
   * [[DeterministicScene DeterministicScene]] and
   * [[monad_core.engine.simulator.EngineFacade EngineFacade]]
   */
    ...
    DeterministicScene(entityCount).map { scene =>
      ...
      () =>
        EngineFacade
          ...
          .left
          .map(error => EnginePerformanceError(error.message))
          .map(_ => ())
    }
