...
object AgentInfo:
  ...
  def from(provider: String, modelName: String): Either[BaseError, AgentInfo] =
    if provider.trim.isEmpty then Left(InvalidProviderName())
    else if modelName.trim.isEmpty then Left(InvalidModelName())
    else Right(AgentInfo(provider, modelName))
