...
case class InvalidCorrectChooses(correctChooses: Int, on:Int)
  extends BaseError(s"correctChooses [$correctChooses] cannot be greater then on [$on], and they should be greater then 0")
...
object AgentEvaluationResult:
  ...
  def fromCorrectChooses(correctChooses: Int, on: Int): Either[InvalidCorrectChooses, AgentEvaluationResult.CorrectChooses] =
    Either.cond(
