...
enum RetrievablePackage(val path: Path):
  ...
  case SimulatorApplication
      extends RetrievablePackage(Paths.get(s"${BasePaths.simulatorPackage}/application"))

  case SimulatorInfrastructure
      extends RetrievablePackage(Paths.get(s"${BasePaths.simulatorPackage}/infrastructure"))

  case SimulatorPresentation
      extends RetrievablePackage(Paths.get(s"${BasePaths.simulatorPackage}/presentation"))

  case SimulatorDomain
      extends RetrievablePackage(Paths.get(s"${BasePaths.simulatorPackage}/domain"))
