...
trait ArchitectureChecker extends AnyFunSuite:
  ...
  private def formatViolations(violations: Map[Path, List[String]], checkedPackage: RetrievablePackage): String =
    violations.map { case (file, imports) =>
      val fileRelative = checkedPackage.path.relativize(file)
      val formattedImports = imports.map(i => "      " + i).mkString("\n")
      s"  - $fileRelative:\n$formattedImports"
    }.mkString("\n")

  def notContainImportWithExceptions(forbiddenSubstring: String, packageExceptions: Set[String]): Matcher[RetrievablePackage] =
    Matcher { packageToCheck =>
      val violations = checkImports(packageToCheck)(
        imp =>
