...
object MonadCoreEntityArranger:
  ...
  def arrangeRedEntity(shapeKind: ShapeKind = ShapeKind.Circle, withOptionals: Boolean = false): MonadCoreEntity =
    if withOptionals
    then
      arrangeEntityWithOptionals(RedEntityId, shapeKind)
    else
      arrangeEntityWithoutOptionals(RedEntityId, shapeKind)


  def arrangeBlueEntity(shapeKind: ShapeKind = ShapeKind.Circle, withOptionals: Boolean = true): MonadCoreEntity =
    if withOptionals
    then
      arrangeEntityWithOptionals(BlueEntityId, shapeKind)
    else
      arrangeEntityWithoutOptionals(BlueEntityId, shapeKind)