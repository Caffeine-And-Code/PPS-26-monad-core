...
import monad_core.engine.physics.helper.PhysicsEntityHelper.{makeFixedEntityCircle, makeMovingEntityCircle}
import org.scalatest.EitherValues.convertEitherToValuable
import org.scalatest.OptionValues.convertOptionToValuable
import org.scalatest.funsuite.AnyFunSuite
import org.scalatest.matchers.should.Matchers

class CollisionResolverTest extends AnyFunSuite
  with Matchers :
  ...
  test("the function should return an error when resolving a collision with one mobile entity without mass"):
  ...
  test("the function should return an error when resolving a collision with two mobiles entities without mass"):
  ...
  test("the function should return a list containing all updated entities when resolving a collision with one mobile entity and one fixed entity"):
  ...
  test("the function should return a list containing all updated entities when resolving a collision with two mobile entities"):

    val entity1 = makeMovingEntityCircle(id = "entity1", speed = Vector2D(1, 0)).withWeight(1).value
    val entity2 = makeMovingEntityCircle(id = "entity2", speed = Vector2D(-1, 0)).withWeight(1).value
  ...
  test("the function should return a list containing all updated entities when resolving multiple collisions"):
