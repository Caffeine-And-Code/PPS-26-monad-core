...
import monad_core.simulator.application.engine.world.{SaveEntityCommand, SaveSurfaceCommand, SaveTeamCommand}
import monad_core.simulator.domain.engine.MonadCoreShape.SimulationCircle
import monad_core.simulator.domain.engine.{MonadCoreEntity, MonadCoreScene, MonadCoreSurface, MonadCoreTeam}
...
class WorldTest extends AnyFunSuite with Matchers with Inside:
  ...
  val updatedSimulationEntity: MonadCoreEntity = MonadCoreEntity(id, position, SimulationCircle(updatedRadius))

  val baseSimulationSurface: MonadCoreSurface = MonadCoreSurface(id, position, SimulationCircle(radius))
  val updatedSimulationSurface: MonadCoreSurface = MonadCoreSurface(id, position, SimulationCircle(updatedRadius))
