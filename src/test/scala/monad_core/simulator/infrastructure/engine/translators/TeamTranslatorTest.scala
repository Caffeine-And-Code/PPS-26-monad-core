...
class TeamTranslatorTest extends AnyFunSuite with Matchers with Inside:
  ...
  test(
    "toEngineModel converts a valid simulation team without enemies to a engine team in a correct way"
  ):
  ...
  test(
    "toEngineModel converts a valid simulation team with enemies to a engine team in a correct way"
  ):
