...
import helpers.arrangers.MonadCoreEntityArranger.{BlueEntityId, DefaultHealth, DefaultRectangleHeight, DefaultRectangleWidth, DefaultSpeed, DefaultTeamId, DefaultWeight, RedEntityId}
...
class EntityTranslatorTest extends AnyFunSuite with Matchers with Inside:
  ...
  val engineEntityWithOptionals: Entity =
    (for
      base       <- Entity.circle(BlueEntityId, Vector2D(0, 0), radius = MonadCoreEntityArranger.DefaultCircleRadius)
  ...
  test("toSimulationEntity converts an engine circle entity without any optional field to a simulation entity correctly"):
    val engineEntity = Entity.circle(RedEntityId, Vector2D(0, 0), radius = MonadCoreEntityArranger.DefaultCircleRadius).value
  ...
  test("toSimulationEntity converts an engine rectangle entity without any optional field to a simulation entity correctly"):
    val engineEntity = Entity.rectangle(RedEntityId, Vector2D(0, 0), height = DefaultRectangleHeight, length = DefaultRectangleWidth).value
  ...
  test("toSimulationEntity converts an engine entity with optional fields to a simulation entity correctly"):
  ...
  test("toEngineModel converts a valid simulation circle entity without optional fields to an engine entity in a correct way"):
    val expectedEngineEntity = Entity.circle(RedEntityId, Vector2D(0, 0), radius = MonadCoreEntityArranger.DefaultCircleRadius).value
  ...
  test("toEngineModel converts a valid simulation rectangle entity without optional fields to an engine entity in a correct way"):
    val expectedEngineEntity = Entity.rectangle(RedEntityId, Vector2D(0, 0), height = DefaultRectangleHeight, length = DefaultRectangleWidth).value
  ...
  test("toEngineModel converts a valid simulation entity with optional fields to an engine entity in a correct way"):
  ...
  test("toEngineModel propagates the error when an optional field is invalid"):
    val simulationEntity = MonadCoreEntityArranger.arrangeRedEntity(ShapeKind.Circle).copy(weight = Some(-5))
