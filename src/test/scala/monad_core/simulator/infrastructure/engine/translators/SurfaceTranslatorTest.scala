...
import helpers.arrangers.MonadCoreSurfaceArranger.{BlueSurfaceId, DefaultAppliedForce, DefaultFrictionIndex, DefaultRectangleHeight, DefaultRectangleWidth, RedSurfaceId}
...
class SurfaceTranslatorTest extends AnyFunSuite with Matchers with Inside:
  ...
  val engineSurfaceWithOptionals: Surface =
    (for
      base <- Surface.circle(BlueSurfaceId, Vector2D(0, 0), radius = MonadCoreSurfaceArranger.DefaultCircleRadius)
      withFriction <- base.withFrictionIndex(DefaultFrictionIndex)
      withForce <- withFriction.withAppliedForce(Vector2D(DefaultAppliedForce._1, DefaultAppliedForce._2))
    yield withForce).value

  test("toSimulationSurface converts an engine circle surface without any optional field to a simulation surface correctly"):
    val engineSurface = Surface.circle(RedSurfaceId, Vector2D(0, 0), radius = MonadCoreSurfaceArranger.DefaultCircleRadius).value
  ...
  test("toSimulationSurface converts an engine rectangle surface without any optional field to a simulation surface correctly"):
    val engineSurface = Surface.rectangle(RedSurfaceId, Vector2D(0, 0), height = DefaultRectangleHeight, length = DefaultRectangleWidth).value
  ...
  test("toSimulationSurface converts an engine surface with optional fields to a simulation surface correctly"):
  ...
  test("toEngineModel converts a valid simulation circle surface without optional fields to an engine surface in a correct way"):
    val expectedEngineSurface = Surface.circle(RedSurfaceId, Vector2D(0, 0), radius = MonadCoreSurfaceArranger.DefaultCircleRadius).value
  ...
  test("toEngineModel converts a valid simulation rectangle surface without optional fields to an engine surface in a correct way"):
    val expectedEngineSurface = Surface.rectangle(RedSurfaceId, Vector2D(0, 0), height = DefaultRectangleHeight, length = DefaultRectangleWidth).value
  ...
  test("toEngineModel converts a valid simulation surface with optional fields to an engine surface in a correct way"):
