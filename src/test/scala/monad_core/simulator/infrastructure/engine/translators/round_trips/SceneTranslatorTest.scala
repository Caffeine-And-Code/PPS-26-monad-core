...
class SceneTranslatorTest extends AnyFunSuite with Matchers with Inside:
  ...
  private def entity(id: String, shapeKind: ShapeKind, withOptionals: Boolean): Entity =
    val base = shapeKind match
      case Circle =>
        Entity.circle(id, Vector2D(0, 0), radius = MonadCoreEntityArranger.DefaultCircleRadius)
      case Rectangle =>
        Entity.rectangle(
          id,
          Vector2D(0, 0),
          height = MonadCoreEntityArranger.DefaultRectangleHeight,
          length = MonadCoreEntityArranger.DefaultRectangleWidth
        )
    ...
    val either =
      if withOptionals then
        for
          e <- base
          withSpeed = e.withSpeed(
            Vector2D(
              MonadCoreEntityArranger.DefaultSpeed._1,
              MonadCoreEntityArranger.DefaultSpeed._2
            )
          )
  ...
  private def surface(id: String, shapeKind: ShapeKind, withOptionals: Boolean): Surface =
    val base = shapeKind match
      case Circle =>
        Surface.circle(id, Vector2D(0, 0), radius = MonadCoreSurfaceArranger.DefaultCircleRadius)
      case Rectangle =>
        Surface.rectangle(
          id,
          Vector2D(0, 0),
          height = MonadCoreSurfaceArranger.DefaultRectangleHeight,
          length = MonadCoreSurfaceArranger.DefaultRectangleWidth
        )
      ...
      if withOptionals then
        for
          ...
          withForce <- withFriction.withAppliedForce(
            Vector2D(
              MonadCoreSurfaceArranger.DefaultAppliedForce._1,
              MonadCoreSurfaceArranger.DefaultAppliedForce._2
            )
          )
        yield withForce
      else base
  ...
  test("SceneTranslator Round Trip property is respected for Simulator Scenes"):
    ...
    forAll(possibleScenes): expectedScene =>
      val translationResult = expectedScene.toEngineModel
        .fold(
